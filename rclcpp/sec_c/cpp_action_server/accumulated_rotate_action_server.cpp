...
private:
    ...
    void execute(const std::shared_ptr<GoalHandleRotate> goal_handle) {
        ...
        while (rclcpp::ok()) {
            double delta_yaw = normalize_angle(current_yaw - previous_yaw);
            accumulated_angle += delta_yaw;
            previous_yaw = current_yaw;

            double remaining = goal->angle - accumulated_angle;
            feedback->remaining_angle = remaining;
            goal_handle->publish_feedback(feedback);

            if (std::abs(remaining) < tolerance) break;

            geometry_msgs::msg::Twist cmd;
            cmd.angular.z = Kp * remaining;
            vel_pub->publish(cmd);
            rate.sleep();
        }
